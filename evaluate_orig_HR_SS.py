...
from utils.config_HR_SS import *
...
def calc_result_excel(result_path, test_label_path, each_file_result=True, today=None):
	result_txt = os.path.join(result_path, os.path.basename(result_path) + '.txt')
	f_txt = open(result_txt, 'wt')
	result_files = glob.glob(result_path + "/*.tif")
	# time_start = datetime.now().strftime('%Y-%m-%d %H:%M"%S')
	time_start = datetime.now(timezone('Asia/Seoul')).strftime('%Y-%m-%d %H:%M:%S')
	print_write_txt(f'Evaluation Start at: {time_start}', f_txt)
	...
	for result_file in result_files:
		...
		ws.cell(row=NUM_CLASSES + 5, column=1, value="Overall Pixel Accuracy")
		ws.cell(row=NUM_CLASSES + 5, column=2, value=accuracy)
		time_end = datetime.now(timezone('Asia/Seoul')).strftime('%Y-%m-%d %H:%M:%S')
		ws.cell(row=NUM_CLASSES + 7, column=1, value="End time")
		ws.cell(row=NUM_CLASSES + 7, column=2, value=time_end)

		wb.save(excel_file)

		print_from_table(count_table, os.path.basename(gt_file), f_txt)
	...
	# ws.cell(row=NUM_CLASSES + 4, column=1, value="fIou")
	ws.cell(row=NUM_CLASSES + 5, column=1, value="Overall Pixel Accuracy")
	# ws.cell(row=NUM_CLASSES + 6, column=1, value="Target Acc")
	# ws.cell(row=NUM_CLASSES + 3, column=2, value=mIou)
	# ws.cell(row=NUM_CLASSES + 4, column=2, value=fIou)
	ws.cell(row=NUM_CLASSES + 5, column=2, value=accuracy)
	...
	print(f'Result : {mIou}, {fIou}, {accuracy}')
	print(f'Result All : {mIou}, {fIou}, {accuracy}, {str(iou_list)[1:-1]}, {str(acc_list)[1:-1]}')
 
	time_end = datetime.now(timezone('Asia/Seoul')).strftime('%Y-%m-%d %H:%M:%S')
	print_write_txt(f'Evaluation End at: {time_end}', f_txt)
